...
def qa_user(User):
    ...
    user = User.objects.create_user(
        email="qa@occipital.com", username="qa", is_staff=True, password="password"
    )
    set_trans_permissions(
        user,
        [
...
def Translation():
    ...
    Translation.objects.create(txt_original="Original", status=STATUS_IN_QUEUE)
    Translation.objects.create(txt_original="Original11", status=STATUS_IN_QUEUE)
    Translation.objects.create(txt_original="Original 2", status=STATUS_IN_PROCESS)
    Translation.objects.create(
        txt_original="Ttt", txt_translation="Ddd", status=STATUS_TRANSLATED
    )
    Translation.objects.create(
        txt_original="Ffff", txt_translation="GGGgg", status=STATUS_CHECKING
    )
    return Translation
