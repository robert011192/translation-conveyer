...
class TranslationSerializer(serializers.ModelSerializer):
    ...
    available_actions = serializers.ListField(
        child=AvailableActionSerializer(), read_only=True
    )
    ...
    def to_representation(self, obj):
        ...
        ret["available_actions"] = (
            ...
            + (
                [{"display": "Translate", "action": "translate"}]
                if obj.user_can_translate(user)
                else []
            )
            + (
                [{"display": "Add QA Comment", "action": "qa_comment"}]
                if obj.user_can_add_qa_comment(user)
                else []
            )
        )
        return ret
    ...
    def update(self, instance, validated_data):
        if (
            "from_status" in validated_data
            and instance.status != validated_data["from_status"]
        ):
            raise ValidationError("Status is outdated")

        user = self.context["request"].user
        if "status" in validated_data and not instance.user_can_move_to_status(
            user, validated_data["status"]
        ):
            raise ValidationError("Permission Denied for Status")
        if "qa_comment" in validated_data and not instance.user_can_add_qa_comment(
            user
        ):
            raise ValidationError("Permission Denied for QA Comment")
        if "txt_translation" in validated_data and not instance.user_can_translate(
            user
        ):
