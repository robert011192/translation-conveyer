...
class Translation(models.Model):
    ...
    user_translator = models.ForeignKey(
    ...
    def user_can_move_to_status(self, user, to_status):
        return to_status != self.status and user.has_perm(
            "translations.can_move_from_{}_to_{}".format(self.status, to_status)
        )

    def user_can_translate(self, user, status=None):
        status = status or self.status
        return status == STATUS_IN_PROCESS and user.has_perm(
            "translations.can_change_translation"
        )

    def user_can_add_qa_comment(self, user, status=None):
        status = status or self.status
        return status == STATUS_CHECKING and user.has_perm(
            "translations.can_change_qa_comment"
        )
